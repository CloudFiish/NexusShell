#!/bin/bash
# NexusShell 与 CodeBuddy 通信快速验证脚本 (简化版)
# 使用方法: ./verify-communication-simple.sh
...
if ./src-tauri/.venv/Scripts/python.exe -c "import codebuddy_agent_sdk; print(codebuddy_agent_sdk.__version__)" 2>/dev/null; then
...
TEST_OUTPUT=$(echo '{"command":"query","session_id":"verify_test","prompt":"1+1=?"}' | timeout 120 ./src-tauri/.venv/Scripts/python.exe scripts/codebuddy_bridge.py 2>&1 || true)
