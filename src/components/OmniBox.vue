...
const { executeSkill, isReady, isLoading, startAgent, stopAgent } = useAgent()
...
async function handleAgentToggle() {
  try {
    ...
    console.error('Agent 操作失败:', error)
    errorMessage.value = error instanceof Error ? error.message : 'Agent 操作失败'
    setTimeout(() => {
      errorMessage.value = null
    }, 5000)
  } finally {
...
<template>
  <div class="relative w-full max-w-3xl group">
    <div class="relative flex items-center transition-all duration-300">
      <!-- Agent 启动/停止按钮 -->
      <button
        @click="handleAgentToggle"
        :disabled="isLoading || isLoadingSkill"
        class="absolute left-0 z-10 px-3 py-2.5 rounded-l-xl border border-r-0 transition-all font-mono text-xs font-bold select-none flex items-center space-x-1.5"
        :class="isReady ? 'bg-green-500/20 text-green-400 border-green-500/30 hover:bg-green-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30 hover:bg-red-500/30'"
        :title="isReady ? '停止 Agent' : '启动 Agent'"
      >
        <span v-if="!isLoading && !isLoadingSkill">
          {{ isReady ? '●' : '○' }}
        </span>
        ...
        <span class="hidden sm:inline">{{ isReady ? 'RUN' : 'START' }}</span>
      </button>

      <!-- 状态指示器 -->
      <div class="absolute left-14 pl-4 flex items-center pointer-events-none transition-colors"
      ...
      <!-- 加载指示器 -->
      <div v-if="isLoadingSkill" class="absolute left-24 pointer-events-none">
      ...
      <input
        ...
        class="w-full h-12 pl-24 pr-20 bg-black/20 backdrop-blur-md border border-white/10 rounded-2xl text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-indigo-500/50 focus:bg-black/40 transition-all font-mono shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
